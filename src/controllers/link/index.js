...
async function create(req, res) {
  const { link } = req.body;
  const owner = req.session.auth;
  const slug = Math.random().toString(36).substring(2, 8);
  await Link.create({ slug: slug, link: link, owner: owner })
    .then(function (data) {
      console.log(data);
      res.status(200).json({ message: "Create new Link" });
    })
...
async function update(req, res) {
  ...
  await Link.update({ link }, { where: { id: id } })
    .then(function (data) {
...
async function deleteLink(req, res) {
  await Link.destroy({
    where: {
      [Op.and]: [{ owner: req.session.auth }, { slug: req.body.slug }],
    },
  })
    .then(function (data) {
